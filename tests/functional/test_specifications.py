...
class H5SonataReader:
    ...
    def _population_names_dict(self):
        return {pname: pgroup['0'] for tgroup in self._population_types_dict.values() \
    ...
    @property
    def attributes(self):
        return {key: group[key] for group in self._population_names_dict.values() \
                                for key in group if isinstance(group[key], h5py.Dataset)}
...
def _check_dataset(filepath, dataset, layout):
    ...
    npt.assert_equal(number_of_columns, int(layout['number_of_columns']), err_msg=(
        f'Dataset {dataset.name} in {filepath} has inconsistent number of columns'
    ))

    npt.assert_equal(dataset.dtype, np.dtype(layout['dtype']), err_msg=(
        f'Dataset {dataset.name} in {filepath} has inconsistent dtype'
    ))
