...
def export_endfoot_mesh(endfoot, filepath):
    ...
    try:
        ...
        L.info('Endfoot saved at: {}'.format(filepath))
...
def export_joined_endfeet_meshes(endfoot_iterator, filepath):
    ...
    vectors = np.array([triangle.tolist() for endfoot in endfoot_iterator for triangle in endfoot.coordinates[endfoot.triangles]])
