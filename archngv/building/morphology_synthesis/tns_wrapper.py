...
def create_tns_inputs(tns_data,
    ...
    if attraction_data is None:
        ...
        parameters[PERIVASCULAR_TYPE]['target_ids'] = np.arange(len(endfeet_targets), dtype=np.int32)
    ...
    for tree_type in parameters['grow_types']:
        L.debug('Barcode scaling for type %s has been set to %s',
            tree_type, parameters[tree_type]['barcode_scaling'])
