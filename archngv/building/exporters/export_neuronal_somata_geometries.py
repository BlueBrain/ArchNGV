...
def rodrigues_rotation_matrix(axis, angle):
    ...
    return np.identity(3) + sn * A + (1.0 - cs) * np.linalg.matrix_power(A, 2)
...
def separate_contour_points_in_two_groups(max_dir, min_dir, contour_points):
    ...
    using that plane.
    """
    left_right_mask = np.fromiter(
        (plane_point_sgn(min_dir, max_dir, p) for p in contour_points), dtype=bool
    )
    ...
    if group1_mask.sum() <= group2_mask.sum():
        ...
        return contour_points[group1_mask], contour_points[group2_mask]

    return contour_points[group2_mask], contour_points[group1_mask]
...
def filter_distant_neurite_starts(contour_points, trunk_starts):
    ...
    mask = np.linalg.norm(trunk_starts - centroid, axis=1) <= 2.0 * max_radius
...
def extract_neuronal_somata_information(output_directory, neuronal_microcircuit, map_func=map):
    ...
    data = {
        "max_radii": np.zeros(n_cells, dtype=np.float32),
    ...
    somata_geom_path = os.path.join(
        output_directory, "microcircuit_somata_geometries_by_revolution.h5"
    )
    ...
    np.save(
        radii_path,
        np.column_stack((data["mean_radii"], data["max_radii"], data["min_radii"])),
    )
