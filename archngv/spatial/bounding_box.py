...
import numpy as np
from .inclusion import points_in_rectangle
...
def aabb_point_cloud(points):
    ...
    return (min_coos[0], min_coos[1], min_coos[2],
            max_coos[0], max_coos[1], max_coos[2])
...
def aabb_disk(center, radius, normal):
    ...
    """
    e_v = radius * np.sqrt(1. - normal ** 2)

    return (center[0] - e_v[0],
            center[1] - e_v[1],
            center[2] - e_v[2],
            center[0] + e_v[0],
            center[1] + e_v[1],
...
def aabb_cylinder(cap_center1, cap_center2, radius):
    ...
    e_v = radius * np.sqrt(1. - u_v ** 2 / np.dot(u_v, u_v))

    return (min(cap_center1[0] - e_v[0], cap_center2[0] - e_v[0]),
